...
def run_inference(model, embs, a_path, b_path, img_size, threshold, device):
    ...
    return changed_pixels, total_pixels, max_prob, mean_prob
...
def main():
    ...
    with open(args.out_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['filename', 'true_label', 'changed_pixels', 'total_pixels', 'pred_label', 'max_prob', 'mean_prob'])
        for i, (fn, a_path, b_path, true_label) in enumerate(pairs):
            changed_pixels, total_pixels, max_prob, mean_prob = run_inference(
                model, embs, a_path, b_path, img_size, args.threshold, args.device)
            pred_label = 1 if changed_pixels > 0 else 0
            writer.writerow([fn, true_label, changed_pixels, total_pixels, pred_label, max_prob, mean_prob])
            if (i + 1) % 200 == 0:
                f.flush()
